...
if (existsSync(infraDirectory)) {
  ...
  const requiredServices = ['AWS S3', 'AWS DynamoDB'];
  ...
  for (const removedService of ['AWS SQS FIFO', 'AWS Lambda', 'AWS CloudWatch Logs', 'AWS Batch Fargate', 'AWS ECR', 'AWS VPC', 'AWS KMS']) {
