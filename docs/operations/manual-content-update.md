...
Issue #465のprivate material backfillは、この公開内容更新手順とは別である。複数動画では `infra/` の `diopside-backfill manifest`、`upload-manifest`、`enqueue`、`report` を運用者が明示的に実行する。AWS基盤は `.github/workflows/deploy-ingestion-infra.yml` から検証済みmainを人が明示デプロイでき、1動画だけのSQS投入は `.github/workflows/enqueue-ingestion-video.yml` から人が明示実行できる。基盤deployは `DEPLOY`、1動画投入は11文字 `video_id` と `ENQUEUE` を必須とし、どちらも共通の `private-backfill-infra` protected environmentで承認する。両workflowはowner ID・repository IDを含むimmutableな完全一致GitHub OIDC subjectと共有infra roleを使い、操作別inline session policyでdeployを対象CDK bootstrap role、enqueueを対象FIFOへの `sqs:SendMessage` だけへ制限する。費用、利用量、契約条件を確認できない場合はAWS deployまたはenqueueを行わない。backfillは固定manifestまたは明示した1動画だけを処理し、将来動画の探索やscheduled実行を開始しない。1動画の処理が15分以内に完了しない場合はSQS再試行後にDLQへ隔離し、成功として扱わない。
