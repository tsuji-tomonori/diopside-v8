...
- 公開コメント・チャットの一時取得物からは、構造化された`message`または`contentText`の本文だけを読み、投稿者名、投稿者ID、チャンネルID、プロフィール情報を候補へ転記しない。
- 公開候補と正本には、正規化した20～50語、1～100の重要度、入力種別、SHA-256入力指紋、除外規則版、生成規則版、日時、確認状態だけを許可する。生本文やコメント単位の情報を残さない。
- コメントまたはチャットを利用できない場合は字幕等へフォールバックできるが、実際の入力種別を画面に表示し、視聴者コメント由来と偽らない。
- 語句自体に個人情報、攻撃的表現、意図しない固有名、重大なネタバレが残り得るため、自動集約だけで公開せず人が全語を確認する。確認後は一時取得物と未承認候補を破棄する。
