...
Issue #465の過去動画素材バックフィルは公開更新ではない。`infra/` に隔離されたS3、DynamoDB、SQS FIFO、Lambda、CloudWatch Logsだけを使い、customer-managed KMS key、AWS Batch、Fargate、ECR、専用VPC、scheduled実行、新着検出、Pages接続、公開データへの投入を持たない。保存時暗号化は各serviceの標準方式を使い、KMS keyの月額費用とAPI request費用を発生させない。
