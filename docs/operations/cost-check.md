...
- CI、手動運用入口、検証済みmainからの静的成果物生成は、公開リポジトリで追加請求が生じない標準runnerだけを使う。動画確認と候補検出の手動ワークフローは読取専用に限定する。private backfill基盤deployだけはprotected environmentで承認したmainの手動実行として分離し、予定実行、外部生成、独自Pages deployを行わない。
