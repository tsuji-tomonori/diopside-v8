...
公開面では動的API、データベース、検索サービス、認証、AWS、独自ドメインを不要にし、既存のChatGPT／Codex契約以外の月次請求を0円にできる。正本、生成物、公開版をGit履歴で復元できる。内容ハッシュrelease IDで複数の索引・詳細・画面bundleを一世代として分離するため、Pages配信やブラウザキャッシュの更新途中でも異なる世代を混在させない。PRではこの生成差分を持たないため、並行する動画更新同士のrelease ID競合も起きない。有限の非公開素材バックフィルは公開面に接続しない別ADR（ADR-0002）の対象である。
