...
private backfill stackはcustomer-managed KMS keyとaliasを作成しない。private S3はbucket defaultのSSE-S3、DynamoDBはAWS所有鍵による標準暗号化を使う。ADR-0006によりSQS、Lambda、worker用CloudWatch Log Groupはstackから廃止し、ローカルworkerとlegacy local importerにはKMS権限を付与しない。
...
1. CloudFormation change setでKMS keyが `Retain` によりtemplate管理外へ残り、aliasが削除されること、S3・DynamoDBの暗号化変更が意図どおりであることを確認する。
2. S3 InventoryまたはHeadObjectで旧keyを参照する全objectを特定し、必要なobjectを同じobject keyへcopyしてSSE-S3へ再暗号化する。
3. objectのbyte数とSHA-256を再読検証し、DynamoDBを含む現行の依存先が旧keyを参照しないことを確認する。
...
AWS deploy、既存objectのcopy、KMS keyの無効化または削除予約、素材upload、公開、mergeはそれぞれ人の明示承認なしに実行しない。
