...
前後編で共通の概要欄・シリーズ共通の出演者名簿は、当該動画への出演証拠として一括適用しない。動画単位で実通話・録画・投稿紹介を確認する。巴本人の登場が確認できない動画は、存在しない登場時刻を作らず確認結果を明記する。

順次紹介区間の人物名を確認できない場合は、名前を省略して根拠のある内容・役割等を章名に使い、公開匿名を維持する。これを理由に推測した人物タグを追加しない。時刻も未支持なら未支持の境界を統合・削除し、疑義・対象区間・採用した対応をPRコメントまたはCommit Commentへ残す。名前の未確認だけで動画全体を止めず、全編根拠と変更後の同一candidate hashに対する独立した事実・編集確認を経て進める。
