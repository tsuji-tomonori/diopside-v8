...
def is_trusted_destination_failure(completed: subprocess.CompletedProcess[str]) -> bool:
    detail = f"{completed.stderr or ''}\n{completed.stdout or ''}".casefold()
    return "trusted-destination" in detail or "trusted destination" in detail
    ...
    *,
    ...
    for attempt in range(1, TRUSTED_DESTINATION_RETRIES + 1):
        ...
        if completed.returncode == 0:
            ...
            break
        trusted_destination = is_trusted_destination_failure(completed)
        log_execution(
            batch_id,
            video_id,
            {
                ...
                "reasoningEffort": reasoning_effort,
                "attempt": attempt,
                "outcome": "retry" if trusted_destination and attempt < TRUSTED_DESTINATION_RETRIES else "failed",
                "reasonCode": "trusted_destination" if trusted_destination else "codex_exec_failed",
                "detailDigest": failure_digest(completed),
            },
        )
        if not trusted_destination or attempt == TRUSTED_DESTINATION_RETRIES:
            reason = "trusted_destination_after_retries" if trusted_destination else "non_retryable"
            raise HarnessError(f"codex exec {role}に失敗しました: {reason}")
...
def process_video(
    ...
    *,
    ...
    draft = json.loads(
        run(
            ["python3", str(AUDIT_SCRIPTS / "validate_candidate.py"), video_id, "--draft-only"],
            env=env,
        ).stdout
    )
    item = load_item(batch_id, video_id)
