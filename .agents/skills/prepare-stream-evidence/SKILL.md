...
3. If `DIOPSIDE_EVIDENCE_REPOSITORY` points to a private evidence repository clone, resolve the video's manifest and reuse checksum-matching captions, audio, and chat before any network acquisition. Reject missing or mismatched files and Git LFS pointers; never treat them as evidence.
4. Before a network acquisition, run `diagnose_youtube_access.py <video-id> --execute`. Keep only its safe public reachability classification and diagnostic digests.
5. For public YouTube captions, inspect and then explicitly retrieve a temporary normalized snapshot:

   `python3 scripts/download_captions.py <video-id>`

   `python3 scripts/download_captions.py <video-id> --execute`

6. For a prepared input snapshot run:
...
7. If captions are unavailable, inspect the public-audio plan with `download_audio.py`; network use requires the human-triggered `--execute` flag. Its execution tries native best-audio and then an MP3 fallback. Run `transcribe_local_asr.py --execute` and pass its output to `prepare_evidence.py`.
8. In a 1 Sol・10 Luna campaign, a Luna acquisition failure returns `needs_sol_recovery`. The parent Sol may rerun ASR with `--bootstrap-local`, which prepares `faster-whisper` and model data only in the ignored batch directory. Do not write `処理不能` for this recoverable failure.
9. After the parent has exhausted the ladder, preserve a safe deferred checkpoint without raw evidence. Continue unrelated lanes.
...
- Persist raw acquisition artifacts only through `scripts/collect_youtube_evidence.py` into the runtime-selected private repository. The public diopside repository still receives no raw evidence or audience identifiers.
